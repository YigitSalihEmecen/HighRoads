/**
 * surface.mjs — checks what the wheels meet along the carriageway.
 *
 * A ray casts straight down at each station. A surface above road level is a
 * step; nothing found is a hole.
 */
...
// every chunk seam and every 2.5 m row — can miss both faces on
// floating-point grounds alone. Verified: 141 misses on the lattice, 0 off it.
  let loadedFor = -1e9;
  for (let s = sLo + 0.137; s <= sHi; s += 0.5) {
    // Re-stream every half-chunk, so each probe runs against the sheets actually
