/**
 * route.mjs — checks the road alignment.
 *
 * Reports the signs of an intentional route: curvature, banking, gradient.
 */
globalThis.document = { createElement: () => ({ style: {}, getContext: () => null }) };
import * as THREE from 'three';
import * as CONFIG from '../src/config.js';
const { WORLD, ROAD, CHUNK } = CONFIG;
// Read at the router's own detail level, so both generators are compared
...
function survey(seed) {
  ...
  // Does the road come back alongside itself? THE structural invariant the cost
  // router made necessary: chunks carry terrain 700 m each side, so nearby
  // roads have sheets that disagree about whose ground it is. Only pairs that
  // can be LOADED AT ONCE matter — beyond `behind+ahead+1` chunks they can never
  // coexist.
  ...
  // Net displacement over arc length. Informational only.
