/**
 * grass.js — the ground cover, and the shader that makes it affordable.
 *
 * This module owns how a tuft is BUILT and DRAWN. Where the tufts go is
 * chunks.js's job, the same split foliage.js already uses: rules and rendering
 * here, placement over there, because placement is the only part that needs the
 * road, the terrain and the streaming.
 *
 ...
 *
 * FADE shrinks a tuft to nothing as it approaches the draw limit, scaled about
 * its own base so it sinks into the ground rather than shrinking to a floating
 * speck. Popping a card out at a distance threshold is visible in exactly the
 * way a car moving toward it makes most obvious.
 */
...
/**
 * Builds everything the grass shares: one geometry, one texture, one material.
 * Every chunk's InstancedMesh points at these, so the whole field is a handful
 * of draw calls and one texture upload for the session.
 */
export function createGrassAssets({ anisotropy = 1 } = {}) {
  ...
  const uniforms = {
    ...
    uFade: { value: new THREE.Vector2(GRASS.fadeStart, GRASS.fadeEnd) },
  };
  ...
  material.onBeforeCompile = (shader) => {
    ...
    shader.vertexShader = shader.vertexShader
      ...
      `)
      // Shrink to nothing with distance, about the base. This is object space
      // and uniform in all three axes, so it is safe to fold into `transformed`
      // before the instance matrix touches it.
      .replace('#include <begin_vertex>', /* glsl */`
        ...
        float fr_fade = 1.0 - smoothstep( uFade.x, uFade.y, fr_d );
        transformed *= fr_fade;
      `)
  ...
  // key, and three keys `onBeforeCompile` materials by this.
  material.customProgramCacheKey = () => 'fastroads-grass';
